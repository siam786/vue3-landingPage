  ...
  <div class="w-full h-auto sliderAx">
    ...
    >
      <div
        ...
        class="object-fill h-[580px] px-20 py-24 text-white bg-cover"
      >
        <div class="md:w-1/2">
          <p class="text-2xl font-bold text-purple-500 uppercase text-stroke">
            {{ slide.category }}
          </p>
          <p class="text-3xl font-bold text-purple-500 text-stroke">
            {{ slide.title }}
          </p>
          <p
          ...
          <a
            href="#"
            class="px-8 py-4 text-xs font-bold text-white uppercase bg-purple-800 rounded hover:bg-gray-200 hover:text-gray-800"
            >Contact us</a
          >
        </div>
      </div>
    ...
    <!-- <div class="flex justify-between w-12 pb-2 mx-auto">
        <button @click="sliderButton(index)" :class="{ 'bg-purple-800': currentSlide === index }" v-for="(slide, index) in slides" :key="index" class="w-4 pb-2 bg-purple-400 rounded-full"></button>
      </div> -->
  </div>
</template>
